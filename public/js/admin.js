  ...
  $.viewReserveInfo = function(resIdx, bus, seat) {
    ...
    $.ajax({
      ...
      success: function(reserveInfo) {
        ...
        var location = '<select name="location[]" class="location">'; $.each(reserveInfo.location, function(i, v) { if (v.short == '') v.short = '승차위치'; cnt = i + 1; if (reserveInfo.reserve.loc == v.no) selected = ' selected'; else selected = ''; location += '<option' + selected + ' value="' + v.no + '">' + v.short + '</option>'; }); location += '</select> ';
