    ...
    <div id="content-wrapper" class="d-flex flex-column">
      <div id="content">
        ...
        <div class="w-100 border mt-2 mb-3 p-3">
          <form id="formSearch" method="get" action="/admin_old/main_list_canceled" class="row align-items-center text-center">
        ...
        <div class="row align-items-center border-bottom mb-3 pb-3">
          ...
          <div class="col-lg-1 text-right"><a href="/admin_old/main_list_canceled?<?=$search['prev']?>">◀</a></div>
          <div class="col-lg-1">
          ...
          <div class="col-lg-1"><a href="/admin_old/main_list_canceled?<?=$search['next']?>">▶</a></div>
          <div class="col-lg-4"></div>
        </div>
<?php
  foreach ($listCancel as $value) {
?>
        <div class="row mb-3">
          <div class="col-lg-11"><b><?=viewStatus($value['status'])?></b> <a target="_blank" href="/admin_old/main_view_progress/<?=$value['idx']?>"><?=$value['subject']?></a><br>산행일시 : <?=$value['startdate']?> (<?=calcWeek($value['startdate'])?>) <?=$value['starttime']?> / 참가비용 : <?=number_format($value['cost_total'] == 0 ? $value['cost'] : $value['cost_total'])?>원</div>
