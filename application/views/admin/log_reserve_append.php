<?php foreach ($listReserve as $value): ?>
  <div class="border-bottom pt-2 pb-2">
    <strong><?=viewStatus($value['notice_status'])?></strong> <a href="<?=BASE_URL?>/admin/main_view_progress/<?=$value['rescode']?>"><?=$value['subject']?></a> - <a href="<?=!empty($value['member_idx']) ? '/admin/member_view/' . $value['member_idx'] : 'javascript:;'?>"><strong class="text-secondary"><?=$value['nickname']?>님</strong></a><br>
    <small>
