      ...
      <div id="content">
        ...
        <form id="formMember" method="post" action="/admin/member_update">
          ...
          <div class="row align-items-center border-bottom mb-3 pb-3">
            ...
            <div class="col-sm-4"><input type="text" readonly name="userid" value="<?=$viewMember['userid']?>" class="form-control"></div>
          </div>
          <div class="row align-items-center border-bottom mb-3 pb-3">
            <div class="col-sm-1">닉네임</div>
            <div class="col-sm-4"><input type="text" name="nickname" value="<?=$viewMember['nickname']?>" class="form-control"></div>
          </div>
          <div class="row align-items-center border-bottom mb-3 pb-3">
            <div class="col-sm-1">실명</div>
            <div class="col-sm-4"><input type="text" name="realname" value="<?=$viewMember['realname']?>" class="form-control"></div>
          </div>
          <div class="row align-items-center border-bottom mb-3 pb-3">
            <div class="col-sm-1">전화번호</div>
            <div class="col-sm-4 row">
              <div class="pl-3"><input type="text" size="4" name="phone1" value="<?=$viewMember['phone'][0]?>" class="form-control"></div>
              <div class="pl-1"><input type="text" size="4" name="phone2" value="<?=$viewMember['phone'][1]?>" class="form-control"></div>
              <div class="pl-1"><input type="text" size="4" name="phone3" value="<?=$viewMember['phone'][2]?>" class="form-control"></div>
            </div>
          </div>
          <div class="row align-items-center border-bottom mb-3 pb-3">
          ...
          <div class="row align-items-center border-bottom mb-3 pb-3">
            ...
            <div class="col-sm-4">
              <label><input<?=$viewMember['level'] == LEVEL_LIFETIME ? ' checked' : ''?> type="checkbox" name="level" value="<?=LEVEL_LIFETIME?>"> 평생회원</label><br>
              <label><input<?=$viewMember['level'] == LEVEL_FREE ? ' checked' : ''?> type="checkbox" name="level" value="<?=LEVEL_FREE?>"> 무료회원</label><br>
              <label><input<?=$viewMember['level'] == LEVEL_DRIVER ? ' checked' : ''?> type="checkbox" name="level" value="<?=LEVEL_DRIVER?>"> 드라이버</label><br>
              <label><input<?=$viewMember['level'] == LEVEL_DRIVER_ADMIN ? ' checked' : ''?> type="checkbox" name="level" value="<?=LEVEL_DRIVER_ADMIN?>"> 드라이버 관리자<br>
              <label><input<?=$viewMember['level'] == LEVEL_BLACKLIST ? ' checked' : ''?> type="checkbox" name="level" value="<?=LEVEL_BLACKLIST?>"> 블랙리스트<br>
              <label><input<?=$viewMember['admin'] == 1 ? ' checked' : ''?> type="checkbox" name="admin" value="1"> 관리자</label><br>
            </div>
          </div>
          <div class="row align-items-center border-bottom mb-3 pb-3">
            <div class="col-sm-1">등급</div>
            <div class="col-sm-4"><?=$viewMember['memberLevel']['levelName']?></div>
          </div>
          <div class="row align-items-center border-bottom mb-3 pb-3">
            <div class="col-sm-1">예약횟수</div>
            <div class="col-sm-4"><?=$viewMember['rescount']?></div>
          </div>
          <div class="row align-items-center border-bottom mb-3 pb-3">
            <div class="col-sm-1">포인트</div>
            <div class="col-sm-4 row align-items-center">
              <div class="col-sm-5"><?=$viewMember['point']?></div>
              <div class="col-sm-4">
          ...
          <div class="row align-items-center border-bottom mb-3 pb-3">
            ...
            <div class="col-sm-4 row align-items-center">
              <div class="col-sm-5"><?=$viewMember['penalty']?></div>
              <div class="col-sm-4">
