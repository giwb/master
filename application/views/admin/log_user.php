      ...
      <div id="content">
        ...
        <?php foreach ($listHistory as $value): ?>
          <div class="border-top pt-3 pb-3 pl-2 pr-2">
            <strong><?=$value['header']?></strong> <?=$value['subject']?> - <a target="_blank" href="<?=!empty($value['userid']) ? base_url() . 'admin/member_view/' . $value['userid'] : 'javascript;'?>" class="text-secondary"><strong><?=$value['nickname']?>님</strong></a>
