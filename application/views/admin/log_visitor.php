<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
        ...
        <div id="content" class="mb-5">
          <div class="w-100 border mt-3 mb-3 p-3">
            <form id="formSearch" method="get" action="<?=BASE_URL?>/admin/log_visitor" class="m-0">
              ...
              <div class="row align-items-center justify-content-center">
                ...
                <div class="pl-3"><button type="button" class="btn btn-sm btn-default btn-search-visitor">검색</button></div>
          ...
          <div class="row align-items-center pt-2 pb-2 bg-secondary text-white">
            <div class="col-sm-2 pl-3 pr-0">접속시간</div>
            <div class="col-sm-2 pl-0 pr-0">닉네임/IP</div>
            <div class="col-sm-4 pl-0 pr-0">링크된곳</div>
            <div class="col-sm-4 pl-0 pr-0">브라우저</div>
          </div>
          <?php foreach ($listVisitor as $value): ?>
          <div class="row align-items-center border-bottom pt-2 pb-2 small visitor-data">
            <div class="col-sm-2 pl-3 pr-0"><?=calcStoryTime($value['created_at'])?></div>
            <div class="col-sm-2 pl-0 pr-0"><?=!empty($value['nickname']) && empty($value['quitdate']) ? '<a href="/admin/member_view/' . $value['created_by'] . '">' . $value['nickname'] . '</a>' : $value['ip_address']?></div>
            <div class="col-sm-4 pl-0 pr-0"><a target="_blank" href="<?=$value['http_referer']?>"><?=strlen($value['http_referer']) > 35 ? substr($value['http_referer'], 0, 35) . '...' : $value['http_referer']?></a></div>
            <div class="col-sm-4 pl-0 pr-0" title="<?=$value['user_agent']?>"><?=getUserAgent($value['user_agent'])?></div>
          </div>
          <?php endforeach; ?>
          <div class="mb-5"></div>
        </div>
