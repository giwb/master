      ...
      <div class="club-main">
        ...
        <div class="sub-content">
          ...
          <?php foreach ($listAlbum as $key => $value): ?>
            ...
            <div class="col-sm-4 text-center mb-2 gallery-item"><a href="javascript:;" class="btn-gallery" data-idx="<?=$value['idx']?>"><img class="gallery-photo border mb-2" src="<?=$value['photo']?>"></a><br><?=$value['subject']?><br><?=$value['nickname']?>님 | <span class="small"><?=date('Y-m-d H:i', $value['created_at'])?><?=$value['created_by'] == $userIdx || !empty($adminCheck) ? ' | <a href="' . base_url() . 'club/album_upload/' . $view['idx'] . '?n=' . $value['idx'] .'">수정</a>' : ''?></span></div>
          <?php endforeach; ?>
