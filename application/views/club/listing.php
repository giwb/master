...
<section id="subpage" class="container">
  <div class="row align-items-center">
    ...
    <div class="col-sm-4 text-right">
      <form method="get" action="/club">
        <input type="hidden" name="s" value="title"><input type="text" size="10" name="k" value="<?=$search == 'title' ? $keyword : ''?>"> <button type="submit" class="btn btn-sm btn-primary">검색</button>
        <a href="/club/entry"><button type="button" class="btn btn-sm btn-primary">등록</button></a>
      </form>
    </div>
  </div>
  <div class="row align-items-top mb-5">
    ...
    <div class="col-sm-8">
      <h4><strong><?=$searchTitle['name']?></strong></h4><hr>
      <ul>
<?php
  if (empty($list)) {
?>
...
?>
        <li class="row align-items-top mb-3">
          <div class="col-4 col-sm-2 pr-0"><a target="_blank" href="<?=$value['domain']?>"><img src="<?=PHOTO_URL?><?=$value['photo']?>" width="100" height="70"></a></div>
          <div class="col-8 col-sm-10 text-justify"><a target="_blank" href="<?=$value['domain']?>"><strong><?=$value['title']?></strong></a><div class="small"><?=ksubstr(strip_tags(reset_html_escape($value['about'])), 100)?></div></div>
        </li>
<?php
    }
  }
