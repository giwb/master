<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
    ...
    <section class="container">
      ...
      <form id="myForm" method="post" action="/club/insert" enctype="multipart/form-data">
        ...
        <div class="row align-items-center border-bottom mb-3 pb-3">
          <div class="col-sm-3 font-weight-bold">페이지 주소 <span class="text-require">(*)</span></div>
          <div class="col-sm-9">
            <div class="row align-items-center">
              <div class="col-6 col-sm-3 p-1 text-right"><?=base_url()?></div>
              <div class="col-6 col-sm-9 p-0 check-url"><input type="text" name="url" value="<?=$view['url']?>" class="form-control form-control-sm"></div>
            </div>
          </div>
        </div>
        <div class="row align-items-center border-bottom mb-3 pb-3">
    ...
    <?php endforeach; ?>
      ...
      if ($('input[name=url]').val() == '') {
        $.openMsgModal('페이지 주소는 꼭 입력해주세요.');
        return false;
      }
      if ($('.check-url img').hasClass('check-url-complete') == false) {
        $.openMsgModal('페이지 주소를 확인해주세요.');
        return false;
      }
      $(this).css('opacity', '0.5').prop('disabled', true).text('잠시만 기다리세요...');
      $('#myForm').submit();
    }).on('blur', '.check-url', function() {
      // 페이지 주소 중복 체크
      var $dom = $(this);
      ...
      if (url != '') {
        if (url.length < 3 || url.length > 20 || !pattern.test(url) || url.search(/\s/) != -1) {
          $.openMsgModal('페이지 주소는 띄어쓰기 없이<br>3자 ~ 20자 이하의 영어 소문자만 가능합니다.');
          $('input[name=url]').val('');
          return false;
        }
        $.ajax({
          url: '/welcome/check_url',
          data: 'url=' + url,
          dataType: 'json',
          type: 'post',
          success: function(result) {
            if (result.error == 1) {
              $.openMsgModal('이미 사용중인 페이지 주소 입니다.');
              $('input[name=url]').val('');
            }
          }
        });
