<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
      ...
      <div class="club-right">
        ...
        <div class="list-schedule">
          ...
          <a href="<?=BASE_URL?>/reserve/<?=$value['idx']?>"><?=viewStatus($value['status'])?> <strong><?=$value['subject']?></strong><br><small><?=$value['startdate']?> (<?=calcWeek($value['startdate'])?>) <?=$value['starttime']?> / <?=number_format($value['cost_total'] == 0 ? $value['cost'] : $value['cost_total'])?>원 / <?=cntRes($value['idx'])?>명</small></a>
          <?php endforeach; ?>
        </div>
        <?php endif; ?>
