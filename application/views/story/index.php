<?php
  foreach ($listStory as $value):
    ...
    if (!empty($value['filename']) && file_exists(PHOTO_PATH . $value['filename'])) {
      ...
      $value['file_thumb'] = PHOTO_URL . 'thumb_' . $value['filename'];
      $value['filename'] = PHOTO_URL . $value['filename'];
    } else {
      $value['file_width'] = 0;
      $value['file_height'] = 0;
      $value['file_thumb'] = '';
          ...
          <article id="post-<?=$value['idx']?>">
            <div class="story-container">
              ...
              <div class="story-content">
                <?php if (!empty($value['file_thumb'])): ?><img class="story-photo" src="<?=$value['file_thumb']?>" data-filename="<?=$value['filename']?>" data-width="<?=$value['file_width']?>" data-height="<?=$value['file_height']?>"><br><?php endif; ?>
