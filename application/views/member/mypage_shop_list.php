    ...
    <div class="border">
      ...
      <div class="pt-2 pb-2 pl-3 pr-3 font-weight-normal">
        ...
        <div class="row align-items-center mt-2">
          ...
          <div class="col-9 col-sm-10 pl-1"><a href="<?=BASE_URL?>/shop/item/<?=$item['idx']?>"><?=$item['name']?></a><br><small><?=!empty($item['option']) ? $item['option'] . ' - ' : ''?><?=number_format($item['amount'])?>개, <?=number_format($item['cost'] * $item['amount'])?>원</small></div>
        </div>
        <?php endforeach; ?>
      </div>
    </div>
