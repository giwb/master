...
// 관리 페이지 클래스
class Admin_old extends Admin_Controller
{
  function __construct()
  {
    ...
    $this->load->model(array('admin_old_model', 'area_model', 'club_model', 'file_model', 'member_model', 'shop_model'));
  }
  ...
  public function index()
  {
    ...
    // 등록된 산행 목록
    $viewData['listNotice'] = $this->admin_old_model->listNotice();

    // 캘린더 설정
    $listCalendar = $this->admin_old_model->listCalendar();
    ...
    // 현재 회원수
    $viewData['cntTotalMember'] = $this->admin_old_model->cntTotalMember();
    // 다녀온 산행횟수
    $viewData['cntTotalTour'] = $this->admin_old_model->cntTotalTour();
    // 다녀온 산행 인원수
    $viewData['cntTotalCustomer'] = $this->admin_old_model->cntTotalCustomer();
    // 오늘 방문자수
    $viewData['cntTodayVisitor'] = $this->admin_old_model->cntTodayVisitor($clubIdx);

    // 댓글
    $paging['perPage'] = 10; $paging['nowPage'] = 0;
    $viewData['listReply'] = $this->admin_old_model->listReply($paging);
    $viewData['listReply'] = $this->load->view('admin_old/log_reply_append', $viewData, true);

    $this->_viewPage('admin_old/index', $viewData);
  }
  ...
  public function reserve_information()
  {
    ...
    $viewData['view'] = $this->admin_old_model->viewEntry($idx);

    if (!empty($viewData['idx'])) {
      $result['reserve'] = $this->admin_old_model->viewReserve($viewData);
  ...
  public function reserve_complete()
  {
    ...
    // 산행 정보
    $viewEntry = $this->admin_old_model->viewEntry($idx);
    ...
    foreach ($arrSeat as $key => $seat) {
      ...
      if (!empty($nowUserid)) {
        ...
        $nowUserInfo = $this->admin_old_model->viewMember($search);
      }
      ...
      // 선택한 좌석 예약 여부 확인
      $checkReserve = $this->admin_old_model->checkReserve($idx, $nowBus, $nowSeat);

      if (empty($resIdx)) {
        if (empty($checkReserve['idx'])) {
          $result = $this->admin_old_model->insertReserve($postData);
        ...
        if (!empty($checkReserve['idx']) && $checkReserve['idx'] != $resIdx) {
          // 기존 예약정보 불러오기
          $search['idx'] = $resIdx;
          $viewOldReserve = $this->admin_old_model->viewReserve($search);
          ...
          if ($checkReserve['status'] == RESERVE_WAIT) {
            ...
            $this->admin_old_model->updateReserve($updateWait, $checkReserve['idx']);
            ...
            $this->admin_old_model->updateReserve($changeData, $checkReserve['idx']);
          }
        } else {
          if ($checkReserve['status'] == RESERVE_WAIT) {
            // 대기자 입력이기 때문에 미입금으로 변경
        ...
        // 예약정보 수정
        $result = $this->admin_old_model->updateReserve($postData, $resIdx);
      }
    }
    ...
    if (empty($result)) {
      ...
      if (!empty($priorityIdx)) {
        ...
        $this->admin_old_model->updateReserve($updateValues, $priorityIdx[1]);
        $updateValues['priority'] = $priorityIdx[1];
        $this->admin_old_model->updateReserve($updateValues, $priorityIdx[0]);
      }
      if ($viewEntry['status'] == STATUS_ABLE) {
        $cntReservation = $this->admin_old_model->cntReservation($idx);
        if ($cntReservation['CNT'] >= 15) {
          // 예약자가 15명 이상일 경우 확정으로 변경
          $updateValues = array('status' => STATUS_CONFIRM);
          $this->admin_old_model->updateEntry($updateValues, $idx);
        }
      }
  ...
  public function reserve_cancel()
  {
    ...
    // 예약 정보
    $viewReserve = $this->admin_old_model->viewReserve($inputData);

    // 회원 정보
    $search['userid'] = $viewReserve['userid'];
    $userData = $this->admin_old_model->viewMember($search);

    // 산행 정보
    $viewEntry = $this->admin_old_model->viewEntry($viewReserve['rescode']);

    // 해당 산행과 버스의 예약자 수
    $cntReservation = $this->admin_old_model->cntReservation($viewReserve['rescode'], $viewReserve['bus'], 1);

    // 대기자 수
    $cntWait = $this->admin_old_model->cntWait($viewReserve['rescode']);
    ...
    if ($cntReservation['CNT'] >= $maxSeat[$viewReserve['bus']] || $cntWait['cnt'] >= 1) {
      ...
      $rtn = $this->admin_old_model->updateReserve($updateValues, $inputData['idx']);
    } else {
      // 좌석이 남아있을 경우에는 그냥 삭제
      $rtn = $this->admin_old_model->deleteReserve($inputData['idx']);

      if ($viewEntry['status'] == STATUS_CONFIRM) {
        $cntReservation = $this->admin_old_model->cntReservation($viewReserve['rescode']);
        if ($cntReservation['CNT'] < 15) {
          // 예약자가 15명 이하일 경우 예정으로 변경
          $updateValues = array('status' => STATUS_ABLE);
          $this->admin_old_model->updateEntry($updateValues, $viewReserve['rescode']);
        }
      }
    }
  ...
  public function reserve_deposit()
  {
    ...
    $viewReserve = $this->admin_old_model->viewReserve($viewData);
    $viewEntry = $this->admin_old_model->viewEntry($viewReserve['rescode']);

    if ($viewReserve['status'] == 1) {
      // 입금취소
      $updateValues['status'] = RESERVE_ON;
      $this->admin_old_model->updateReserve($updateValues, $viewData['idx']);
    ...
    if ((count(array_keys(array_flip($array_seat))) !== count($array_seat)) > 0) {
      ...
      foreach ($reserve_seat as $value) {
        if ($value['seat'] > 0 && $value['seat'] != $value['origin_seat']) {
          ...
          $this->admin_old_model->updateReserve($updateData, $value['idx']);
        }
      }
  ...
  public function main_list_progress()
  {
    ...
    $viewData['list'] = $this->admin_old_model->listNotice($search);
    ...
    $this->_viewPage('admin_old/main_list_progress', $viewData);
  }
  ...
  public function main_list_planned()
  {
    ...
    $this->_viewPage('admin_old/main_list_planned', $viewData);
  }
  ...
  public function main_view_progress($idx)
  {
    ...
    $viewData['view'] = $this->admin_old_model->viewEntry($viewData['rescode']);

    // 버스 형태별 좌석 배치
    $viewData['busType'] = getBusType($viewData['view']['bustype'], $viewData['view']['bus']);

    // 예약 정보
    $viewData['reserve'] = $this->admin_old_model->viewReserve($viewData);

    // 대기자 정보
    $viewData['wait'] = $this->admin_old_model->listWait($viewData['rescode']);
    ...
    // 댓글
    $viewData['listReply'] = $this->admin_old_model->listReply(NULL, $viewData['rescode']);
    $viewData['listReply'] = $this->load->view('admin_old/log_reply_append', $viewData, true);

    $this->_viewPage('admin_old/main_view_progress', $viewData);
  }
  ...
  public function main_view_boarding($idx)
  {
    ...
    foreach ($viewData['busType'] as $key1 => $bus) {
      ...
      foreach ($listLocation as $key2 => $value) {
        ...
        $resData = $this->admin_old_model->listReserveLocation($viewData['rescode'], $busNumber, $value['no']);
    ...
    $this->_viewPage('admin_old/main_view_boarding', $viewData);
  }
  ...
  public function main_view_adjust($idx)
  {
    ...
    $this->_viewPage('admin_old/main_view_adjust', $viewData);
  }
  ...
  public function main_view_sms($idx)
  {
    ...
    // 문자양식 만들기
    $list = $this->admin_old_model->listSMS($idx);
    ...
    $this->_viewPage('admin_old/main_view_sms', $viewData);
  }
  ...
  public function main_view_seat($idx)
  {
    ...
    // 예약 정보
    $viewData['reserve'] = $this->admin_old_model->viewProgress($idx);

    $this->_viewPage('admin_old/main_view_seat', $viewData);
  }
  */
  ...
  public function main_list_closed()
  {
    ...
    $viewData['listClosed'] = $this->admin_old_model->listNotice($viewData['search']);

    $this->_viewPage('admin_old/main_list_closed', $viewData);
  }
  ...
  public function main_list_canceled()
  {
    ...
    $viewData['listCancel'] = $this->admin_old_model->listNotice($viewData['search']);

    $this->_viewPage('admin_old/main_list_canceled', $viewData);
  }
  ...
  public function change_status()
  {
    ...
    if ($updateValues['status'] == STATUS_CLOSED) { // 종료 처리
      $viewEntry = $this->admin_old_model->viewEntry($search['rescode']);
      $viewReserve = $this->admin_old_model->viewReserveClosed($search['rescode']);
      ...
      foreach ($viewReserve as $value) {
        ...
        if ($userData['level'] != 2 && $userData['admin'] != 1) { // 무료회원과 관리자는 적립금 없음
          ...
          $addedReserve = $this->admin_old_model->viewReserveClosedAdded($search['rescode'], $userData['userid']);
          if ($addedReserve['cnt'] > 1) {
            $addedPoint = ($addedReserve['cnt'] - 1) * 1000;
            $userData = $this->admin_old_model->viewMember($search); // 갱신된 정보를 다시 불러옴
      ...
      $viewEntry = $this->admin_old_model->viewEntry($search['rescode']);
      $viewReserve = $this->admin_old_model->viewReserve($search);
    ...
    $rtn = $this->admin_old_model->updateEntry($updateValues, $search['rescode']);
  ...
  public function change_visible()
  {
    ...
    $rtn = $this->admin_old_model->updateEntry($updateData, $idx);
  ...
  public function main_entry($idx=NULL)
  {
    if (!is_null($idx)) {
      $viewData['btn'] = '수정';
      $viewData['view'] = $this->admin_old_model->viewEntry(html_escape($idx));
    ...
    // 산행 목록
    $viewData['listNotice'] = $this->admin_old_model->listNotice(NULL, 'desc');

    // 버스 형태
    $viewData['listBustype'] = $this->admin_old_model->listBustype();
    ...
    $this->_viewPage('admin_old/main_entry', $viewData);
  }
  ...
  public function main_entry_notice()
  {
    ...
    if (!empty($idx)) {
      $result = $this->admin_old_model->viewEntry($idx);
  ...
  public function main_entry_update()
  {
    ...
    if (!$this->input->post('subject') || !$this->input->post('startdate') || !$this->input->post('enddate')) {
      ...
      if (!$idx) {
        // 등록
        $rtn = $this->admin_old_model->insertEntry($postData);
      } else {
        // 수정
        $rtn = $this->admin_old_model->updateEntry($postData, $idx);
      }
...
foreach ($viewData['listNotice'] as $value) {
  ...
  if (!empty($value['plan'])) {
    ...
    $this->admin_old_model->insertNoticeDetail($insertValues);
    $sort++;
  }
...
*/
    ...
    // 산행 공지사항
    $viewData['listNoticeDetail'] = $this->admin_old_model->listNoticeDetail($idx);

    $this->_viewPage('admin_old/main_notice', $viewData);
  }
  ...
  public function main_notice_update()
  {
    ...
    if (!empty($noticeIdx)) {
      // 삭제
      $listNoticeDetail = $this->admin_old_model->listNoticeDetail($noticeIdx);
      foreach ($listNoticeDetail as $value) {
        if (!in_array($value['idx'], $inputData['idx'])) {
          ...
          $this->admin_old_model->updateNoticeDetail($updateValues, $value['idx']);
        }
      }
      ...
      foreach ($inputData['title'] as $key => $value) {
        ...
        if (!empty($idx)) {
          ...
          $this->admin_old_model->updateNoticeDetail($updateValues, $idx);
        } else {
          // 등록
          $insertValues = array(
    ...
    redirect(BASE_URL . '/admin_old/main_notice/' . $noticeIdx);
  }
  ...
  {
    $idx = html_escape($this->input->post('idx'));
    $rtn = $this->admin_old_model->deleteEntry($idx);
  ...
  public function main_wait_insert()
  {
    ...
    if (!empty($search['userid'])) {
      $viewMember = $this->admin_old_model->viewMember($search);
      $insertValues['created_by'] = $viewMember['idx'];
    }

    $rtn = $this->admin_old_model->insertWait($insertValues);
  ...
  {
    $idx = html_escape($this->input->post('idx'));
    $rtn = $this->admin_old_model->deleteWait($idx);
  ...
  public function main_schedule()
  {
    ...
    $viewData['listSchedule'] = $this->admin_old_model->listNotice($viewData['search']);
    ...
    $this->_viewPage('admin_old/main_schedule', $viewData);
  }
  ...
  public function main_schedule_past()
  {
    ...
    if (!empty($idx)) {
      $viewNotice = $this->admin_old_model->viewEntry($idx);
      ...
      if (!empty($viewNotice['idx'])) {
        ...
        $listPastNotice = $this->admin_old_model->listNotice($search);
  ...
  public function main_list_copy()
  {
    ...
    $viewData['listNotice'] = $this->admin_old_model->listNotice($search);
    ...
    $this->load->view('admin_old/main_list_copy', $viewData);
  }
  ...
  public function member_list()
  {
    ...
    $viewData['listMembers'] = $this->admin_old_model->listMembers($paging, $viewData['search']);

    if ($page >= 2) {
      // 2페이지 이상일 경우에는 Json으로 전송
      $result['page'] = $page;
      $result['html'] = $this->load->view('admin_old/member_list_append', $viewData, true);
      $this->output->set_output(json_encode($result));
    } else {
      // 1페이지에는 View 페이지로 전송
      $this->_viewPage('admin_old/member_list', $viewData);
    }
  }
  ...
  public function member_view($keyword)
  {
    ...
    $viewData['view'] = $this->admin_old_model->viewMember($search);
    ...
    $this->_viewPage('admin_old/member_view', $viewData);
  }
  ...
  public function member_update()
  {
    ...
    $result = $this->admin_old_model->updateMember($updateValues, $idx);
  ...
  public function member_update_point($idx)
  {
    ...
    $result = $this->admin_old_model->updateMember($updateValues, $search['idx']);
  ...
  public function member_delete()
  {
    ...
    if (!empty($idx)) {
      ...
      $rtn = $this->admin_old_model->updateMember($updateValues, $idx);
  ...
  {
    $viewData = array();
    $this->_viewPage('admin_old/member_entry', $viewData);
  }
  ...
  public function attendance_list()
  {
    ...
    $dateEnd = date('Y-m-d');
    $viewData['listAttendanceNotice'] = $this->admin_old_model->listAttendanceNotice($dateStart, $dateEnd);
    $maxNotice = count($viewData['listAttendanceNotice']);
    $viewData['width'] = 88 / ($maxNotice);

    // 랭킹별 닉네임 추출
    $viewData['listNickname'] = $this->admin_old_model->listAttendanceNickname();
    ...
    if ($viewData['listNickname'] != '') {
      foreach ($viewData['listNickname'] as $key => $list) {
        ...
        foreach ($viewData['listAttendanceNotice'] as $notice) {
          $viewData['listNickname'][$key]['listNotice'][] = $this->admin_old_model->checkAttendance($notice['idx'], $list['nickname']);
        }
      }
    }

    $this->_viewPage('admin_old/attendance_list', $viewData);
  }
  ...
  public function attendance_mountain()
  {
    ...
    foreach ($viewData['listNickname'] as $key => $value) {
      $listAttendanceResCode = $this->admin_old_model->listAttendanceResCode($value['nickname']);
      $viewData['listNickname'][$key]['mname'] = '';

      foreach ($listAttendanceResCode as $cnt => $data) {
        $getAttendanceMountainName = $this->admin_old_model->getAttendanceMountainName($data['rescode']);
        if ($cnt != 0) $viewData['listNickname'][$key]['mname'] .= ' / ';
        $viewData['listNickname'][$key]['mname'] .= $getAttendanceMountainName['mname'];
      }
    }

    $this->_viewPage('admin_old/attendance_mountain', $viewData);
  }
  ...
  public function attendance_make()
  {
    ...
    $dateStart = '2019-04-06';
    $dateEnd = date('Y-m-d');
    $dataNotice = $this->admin_old_model->listAttendanceNotice($dateStart, $dateEnd);
    $maxNotice = count($dataNotice);
    $listMember = array();

    foreach ($dataNotice as $value) {
      $date = date("ymd", strtotime($value['startdate']));
      $entryMember = $this->admin_old_model->getAttendanceNickname($value['idx']);
    ...
    // 이전 데이터 삭제
    $this->admin_old_model->deleteAttendance();
    ...
    foreach ($listMember as $key => $list) {
      foreach ($list as $nickname) {
        ...
        $rtn = $this->admin_old_model->insertAttendance($insertData);
      }
    }
  ...
  public function attendance_auth()
  {
    ...
    $viewData['listNotice'] = $this->admin_old_model->listNotice($search, 'desc');

    $this->_viewPage('admin_old/attendance_auth', $viewData);
  }
  ...
  public function attendance_auth_insert()
  {
    ...
    $rtn = $this->admin_old_model->insertAttendanceAuth($insertValues);
  ...
  public function log_reserve()
  {
    ...
    // 예약기록 불러오기
    $viewData['listReserve'] = $this->admin_old_model->listReserve($paging, $viewData['keyword']);

    if ($page >= 2) {
      // 2페이지 이상일 경우에는 Json으로 전송
      $result['page'] = $page;
      $result['html'] = $this->load->view('admin_old/log_reserve_append', $viewData, true);
      $this->output->set_output(json_encode($result));
    } else {
      // 1페이지에는 View 페이지로 전송
      $this->_viewPage('admin_old/log_reserve', $viewData);
    }
  }
  ...
  public function log_user()
  {
    ...
    if (!empty($action)) {
      ...
      $viewData['listHistory'] = $this->admin_old_model->listHistory($paging, $viewData);
    } else {
    ...
    $viewData['maxLog'] = $this->admin_old_model->cntHistory($viewData);
    $viewData['pageType'] = 'member';
    $viewData['pageUrl'] = '/admin_old/log_user';
    $viewData['pageTitle'] = '회원 활동기록';

    foreach ($viewData['listHistory'] as $key => $value) {
      if (!empty($value['userid'])) {
        $search_member['userid'] = $value['userid'];
        $viewData['listHistory'][$key]['userData'] = $this->admin_old_model->viewMember($search_member);
      } else {
    ...
    if ($page >= 2) {
      // 2페이지 이상일 경우에는 Json으로 전송
      $result['page'] = $page;
      $result['html'] = $this->load->view('admin_old/log_user_append', $viewData, true);
      $this->output->set_output(json_encode($result));
    } else {
      // 1페이지에는 View 페이지로 전송
      $viewData['listHistory'] = $this->load->view('admin_old/log_user_append', $viewData, true);
      $this->_viewPage('admin_old/log_user', $viewData);
    }
  }
  ...
  public function log_admin()
  {
    ...
    $viewData['pageUrl'] = '/admin_old/log_admin';
    ...
    foreach ($viewData['listHistory'] as $key => $value) {
      ...
      switch ($value['action']) {
        ...
        case LOG_ADMIN_DEPOSIT_CONFIRM: // 관리자 입금확인
          ...
          $viewReserve = $this->admin_old_model->viewReserve($search_reserve);
  ...
  public function log_refund()
  {
    ...
    $viewData['pageUrl'] = '/admin_old/log_refund';
    $viewData['pageTitle'] = '비회원 환불기록';

    foreach ($viewData['listHistory'] as $key => $value) {
      $viewData['listHistory'][$key]['userData']['nickname'] = $value['nickname'];

      $viewEntry = $this->admin_old_model->viewEntry($value['fkey']);
  ...
  public function log_bus()
  {
    ...
    $viewData['pageUrl'] = '/admin_old/log_bus';
    ...
    foreach ($viewData['listHistory'] as $key => $value) {
      ...
      switch ($value['action']) {
        case LOG_DRIVER_CHANGE: // 차량 변경
          ...
          $viewData['listHistory'][$key]['subject'] = '<a href="/admin_old/main_view_progress/' . $value['fkey'] . '">' . $viewEntry['subject'] . '</a> - ' . $value['subject'];
          break;
      }
    }
  ...
  public function log_buy()
  {
    ...
    $viewData['pageUrl'] = '/admin_old/log_buy';
  ...
  public function log_buy_admin()
  {
    ...
    $viewData['pageUrl'] = '/admin_old/log_buy_admin';
  ...
  public function log_check()
  {
    ...
    $viewHistory = $this->admin_old_model->viewHistory($idx);

    if ($viewHistory['status'] == 1) $status = 0; else $status = 1;

    $updateValues = array('status' => $status);
    $rtn = $this->admin_old_model->updateHistory($updateValues, $idx);
  ...
  public function log_reply()
  {
    ...
    if ($page >= 2) {
      // 2페이지 이상일 경우에는 Json으로 전송
      $result['page'] = $page;
      $result['html'] = $this->load->view('admin_old/log_reply_append', $viewData, true);
      $this->output->set_output(json_encode($result));
    } else {
      $viewData['listReply'] = $this->load->view('admin_old/log_reply_append', $viewData, true);
      // 1페이지에는 View 페이지로 전송
      $this->_viewPage('admin_old/log_reply', $viewData);
    }
  }
  ...
  public function log_reply_delete()
  {
    ...
    $this->admin_old_model->deleteReply($updateValues, $idx);
  ...
  {
    $viewData['listReaction'] = $this->admin_old_model->listReaction();

    $this->_viewPage('admin_old/log_reaction', $viewData);
  }
  ...
  public function log_reaction_delete()
  {
    ...
    $result = $this->admin_old_model->deleteReaction($created_by, $created_at);
  ...
  public function log_visitor()
  {
    ...
    $viewData['listVisitor'] = $this->admin_old_model->listVisitor($viewData);

    $this->_viewPage('admin_old/log_visitor', $viewData);
  }
  ...
  public function setup_information()
  {
    ...
    $this->_viewPage('admin_old/setup_information', $viewData);
  }
  ...
  public function setup_information_update()
  {
    ...
    redirect(BASE_URL . '/admin_old/setup_information');
  }
  ...
  public function setup_pages()
  {
    ...
    $this->_viewPage('admin_old/setup_pages', $viewData);
  }
  ...
  public function setup_pages_update()
  {
    ...
    redirect(BASE_URL . '/admin_old/setup_pages');
  }
  ...
  {
    $viewData['listFront'] = $this->admin_old_model->listFront();

    $this->_viewPage('admin_old/setup_front', $viewData);
  }
  ...
  public function setup_front_insert()
  {
    if ($_FILES['file_obj']['type'] == 'image/jpeg') {
      ...
      if (move_uploaded_file($_FILES['file_obj']['tmp_name'], PATH_FRONT . $filename)) {
        $maxIdx = $this->admin_old_model->getFrontSortMaxIdx();
        $insertData = array(
          'sort_idx' => $maxIdx['sort_idx'] + 1,
          'filename' => $filename,
        );
        $this->admin_old_model->insertFront($insertData);
  ...
  public function setup_front_sort()
  {
    ...
    $listFront = $this->admin_old_model->listFront();

    foreach ($listFront as $key => $value) {
      $result = $this->admin_old_model->updateFrontSortIdx($value['filename'], $sortIdx[$key]);
    }
  ...
  public function setup_front_delete()
  {
    ...
    // 데이터 삭제
    $result = $this->admin_old_model->deleteFront($filename);
    ...
    foreach ($listFront as $key => $value) {
      $this->admin_old_model->updateFrontSortIdx($value['filename'], $key + 1);
    }
  ...
  public function setup_sms()
  {
    ...
    $this->_viewPage('admin_old/setup_sms', $viewData);
  }
  ...
  public function setup_bustype()
  {
    ...
    $this->_viewPage('admin_old/setup_bustype', $viewData);
  }
  ...
  public function setup_bustype_add($idx=NULL)
  {
    if (!is_null($idx)) {
      $idx = html_escape($idx);
      $viewData['viewBustype'] = $this->admin_old_model->getBustype($idx);
    ...
    $viewData['listBusdata'] = $this->admin_old_model->listBusdata();

    $this->_viewPage('admin_old/setup_bustype_add', $viewData);
  }
  ...
  public function setup_bustype_insert()
  {
    ...
    $result = $this->admin_old_model->insertBustype($insertData);
  ...
  public function setup_bustype_update()
  {
    ...
    if (!is_null($idx)) {
      ...
      $result = $this->admin_old_model->updateBustype($updateData, $idx);
    }
  ...
  public function setup_bustype_sort()
  {
    ...
    foreach ($arrSort as $key => $value) {
      if (!empty($value)) {
        ...
        $this->admin_old_model->updateBustype($updateValues, $value);
      }
    }
  }
  ...
  public function setup_bustype_hide()
  {
    ...
    if (!is_null($idx)) {
      $getBusType = $this->admin_old_model->getBustype($idx);
      if ($getBusType['visible'] == 'Y') $visible = 'N'; else $visible = 'Y';

      $updateData = array('visible' => $visible);
      $this->admin_old_model->updateBustype($updateData, $idx);
  ...
  public function setup_bustype_delete()
  {
    ...
    $result = $this->admin_old_model->deleteBustype($idx);
  ...
  public function setup_calendar()
  {
    ...
    // 캘린더 설정
    $viewData['listCalendar'] = $this->admin_old_model->listCalendar();

    $this->_viewPage('admin_old/setup_calendar', $viewData);
  }
  ...
  public function setup_calendar_update()
  {
    ...
    if (!empty($idx)) {
      $rtn = $this->admin_old_model->updateCalendar($postData, $idx);
    } else {
      $rtn = $this->admin_old_model->insertCalendar($postData);
    }
  ...
  {
    $idx = html_escape($this->input->post('idx'));
    $rtn = $this->admin_old_model->deleteCalendar($idx);
  ...
  public function search_by_nickname()
  {
    ...
    $rtn = $this->admin_old_model->viewMember($search);
  ...
  private function _viewPage($viewPage, $viewData=NULL)
  {
    ...
    $this->load->view('admin_old/header', $headerData);
    $this->load->view($viewPage, $viewData);
    $this->load->view('admin_old/footer');
  }
}
?>
